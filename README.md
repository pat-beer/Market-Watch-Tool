...
## แท็บ "สะสม" — สัญญาณซื้อถัวเฉลี่ยระยะยาวตาม MA (rules.js → `accumSignal`)
สำหรับพอร์ตสะสม/เกษียณ ไม่ใช่เทรดรายวัน — คำนวณจากราคาจริงล้วนๆ ไม่ใช้ AI จึงไม่ต้องรอ Claude วิเคราะห์
- เทรนด์ถือว่า **ยังไม่เสีย** ตราบใดที่ราคา > เส้น 200D
- ราคาแตะเส้น **50D** (ห่างไม่เกิน ±2%) → ไม้ปกติ
- ราคาแตะเส้น **100D** (ห่างไม่เกิน ±2%) → ไม้ใหญ่กว่า (ราคาถูกกว่า)
- ราคา **หลุดเส้น 200D** → หยุดสะสมชั่วคราว รอเทรนด์ขาขึ้นกลับมาก่อน (ชนะทุกเงื่อนไขอื่น)
รายชื่อสินทรัพย์ที่ใช้ระบบนี้แก้ได้ที่ `watchlist.json` → `accumulate` (แก้ไฟล์ตรงๆ เหมือน `daily`/`groups` ไม่มีปุ่มเพิ่มในแอป)
ปรับ tolerance (±2%) ได้ที่ `ACCUM_TOL` ใน `rules.js`
